            ...
            Some(cmd) = command_rx.recv() => {
                match cmd {
                    ...
                    RelayCommand::Disconnect => {
                        ...
                        inner.write().state = RelayClientState::Disconnected;
                        on_relay_disconnected(&app, &inner);
    ...
    match message {
                ...
                if encrypted {
                    ...
                    match decrypt_cryptojs_aes(&content, &decryption_key) {
                        ...
                        Err(e) => {
                            ...
                            // 通知手机端解密失败，提示重新配对
                            let state = app.state::<crate::state::SharedAppState>();
                            let mac_device_id = state.get_config().device_id;
                            let err_relay = WsMessage::Relay {
                                from: mac_device_id,
                                to: from.clone(),
                            ...
                            let relay_client = app.state::<crate::commands::SharedRelayClient>();
                            let _ = relay_client.read().send(err_relay);
                            return;
                        }
                    }
                }
        ...
        ServerMessage::ClientOnline {
            ...
            if !client_id.contains('_') {
                let state = app.state::<crate::state::SharedAppState>();
                let is_trusted = state.get_config().is_device_trusted(&client_id);
                ...
                if is_trusted {
                    ...
                    tracing::debug!("设备 {} 未配对，忽略上线消息", client_id);
                }
            }
        }
        ...
        ServerMessage::TrustList { devices } => {
            ...
            // 空列表保护：如果服务器返回空列表但本地有设备，跳过同步，防止误清除
            let local_count = state.get_config().trusted_devices.len();
            if devices.is_empty() && local_count > 0 {
                tracing::warn!("[TrustList] 服务器返回空列表，但本地有 {} 台设备，跳过同步", local_count);
                let _ = app.emit("relay:trust_list", &devices);
                return;
            }

            // 收集服务器端设备 ID 集合（用于后续双向同步）
            let server_ids: std::collections::HashSet<String> =
                devices.iter().map(|d| d.id.clone()).collect();

            // 添加/更新服务器端的设备（保留已有 encryption_key）
            ...
            // 双向同步：移除本地有但服务器没有的设备（解除配对后不再回弹）
            state.update_config(|config| {
                let before = config.trusted_devices.len();
                config.trusted_devices.retain(|d| server_ids.contains(&d.id));
                let removed = before - config.trusted_devices.len();
                if removed > 0 {
                    tracing::info!("[TrustList] 移除本地多余设备 {} 台", removed);
                }
            });
...
/// relay 连接断开时的统一清理：清空在线列表并复位托盘图标
fn on_relay_disconnected(app: &AppHandle, inner: &Arc<RwLock<RelayClientInner>>) {
    let had_devices = !inner.read().online_clients.is_empty();
    inner.write().online_clients.clear();
    inner.write().client_last_seen.clear();

    let state = app.state::<SharedAppState>();
    state.clear_online_devices();

    if had_devices {
        let _ = crate::services::tray::update_tray_menu(app);
        let _ = crate::services::tray::update_tray_icon(
            app,
            crate::services::tray::TrayIconType::Disconnected,
        );
    }
    // !had_devices 即 was_empty；断连后 online_clients 已清空，is_empty_now = true
    sync_hotkeys_with_online_status(app, !had_devices, true);
}
