...
impl HotkeyManager {
    ...
    /// 注册单个快捷键
    pub fn register(&self, action: String, accelerator: String) -> Result<(), String> {
        // 先注销旧的快捷键（如果存在）
    ...
    pub fn unregister(&self, action: &str) -> Result<(), String> {
        ...
        if let Some(accelerator) = registered.get(action).cloned() {
            // 走原有逻辑
    ...
    /// 批量注册快捷键
    pub fn register_all(&self, hotkeys: HashMap<String, String>) -> Result<(), String> {
        // 先注销所有旧的快捷键
        self.unregister_all()?;
        ...
        for (action, accelerator) in hotkeys {
            if !accelerator.is_empty() {
                self.register(action, accelerator)?;
            }
        }
    ...
    /// 获取已注册的快捷键列表
    pub fn get_registered(&self) -> HashMap<String, String> {
...
async fn handle_hotkey_press(app: &AppHandle, action: &str) -> Result<(), String> {
    ...
    for target_id in online_clients {
        ...
        if action == "tap" || action == "longpress" {
            // 检查设备平台，iOS 设备跳过模拟点击
            if let Some(platform) = relay_client.read().get_device_platform(&target_id) {
                if platform == "ios" {
                    tracing::info!("跳过 iOS 设备 {} 的模拟点击指令", target_id);
                    continue;
                }
            }
...
fn match_coordinates(
    ...
    for config in &config_list {
        let cw = config.get("width")?.as_f64()?;
        let ch = config.get("height")?.as_f64()?;
